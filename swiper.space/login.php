<?php
	if (isset($_POST["email"])) {
		...
		$getsalt = $connection->prepare("select salt, password, id, first_name from seekers where email=?;");
		...
		if ($password == $result[1]) {
			...
			$alc = $connection->prepare("insert into login_seeker (id, seeker_id, expires) values (?,?, ADDTIME(NOW(), '336:0:0.0'));");
